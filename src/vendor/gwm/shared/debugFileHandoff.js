...
function openHandoffDb() {
  return new Promise((resolve, reject) => {
    ...
    if (!indexedDb) {
      reject(
        new Error(
          '当前浏览器不支持本地文件暂存，请直接打开目标调试页后重新选择文件。',
        ),
      );
      return;
    }
    ...
    request.onerror = () =>
      reject(request.error || new Error('无法打开本地文件暂存。'));
    request.onsuccess = () => resolve(request.result);
  });
}

export async function saveDebugFileHandoff(
  file,
  targetKind = getDebugFileKind(file),
) {
  if (!file || !targetKind) {
  ...
  await new Promise((resolve, reject) => {
    ...
    transaction.onerror = () =>
      reject(transaction.error || new Error('本地文件暂存失败。'));
    transaction.onabort = () =>
      reject(transaction.error || new Error('本地文件暂存已取消。'));
  }).finally(() => db.close());
  return record;
}
...
export async function consumeDebugFileHandoff(expectedKind = null) {
  ...
  await new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(request.error || new Error('读取本地文件暂存失败。'));
    transaction.oncomplete = () => resolve();
    transaction.onerror = () =>
      reject(transaction.error || new Error('读取本地文件暂存失败。'));
    transaction.onabort = () =>
      reject(transaction.error || new Error('读取本地文件暂存已取消。'));
  }).finally(() => db.close());
