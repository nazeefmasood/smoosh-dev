...
const MIN_VISIBLE_CAPTURE_AREA =
  MIN_VISIBLE_CAPTURE_EDGE * MIN_VISIBLE_CAPTURE_EDGE;
const CONTAINER_CAPTURE_AREA_RATIO = 4;
const GEMINI_FULLSCREEN_CONTAINER_SELECTOR =
  'expansion-dialog,[role="dialog"],.image-expansion-dialog-panel,.cdk-overlay-pane';
...
export function getRememberedPreviewResultRegistryEntryForTests(
  sourceUrl = '',
) {
...
function hasExplicitBoundSourceUrl(imageElement, sourceUrl = '') {
  const explicitSourceUrl =
    typeof imageElement?.dataset?.gwrSourceUrl === 'string'
  ...
  return Boolean(
    explicitSourceUrl &&
      normalizedSourceUrl &&
      explicitSourceUrl === normalizedSourceUrl,
  );
}
...
function getComparableImageSize(imageElement) {
  const width =
    Number(imageElement?.naturalWidth) ||
    Number(imageElement?.clientWidth) ||
    Number(imageElement?.width) ||
    0;
  const height =
    Number(imageElement?.naturalHeight) ||
    Number(imageElement?.clientHeight) ||
    Number(imageElement?.height) ||
    0;
  if (width <= 0 || height <= 0) {
    return null;
  }
...
function readAssetIdsFromImageDataset(imageElement) {
  ...
  const responseId =
    typeof imageElement.dataset[PAGE_IMAGE_RESPONSE_ID_KEY] === 'string'
      ? imageElement.dataset[PAGE_IMAGE_RESPONSE_ID_KEY].trim()
      : '';
  const draftId =
    typeof imageElement.dataset[PAGE_IMAGE_DRAFT_ID_KEY] === 'string'
      ? imageElement.dataset[PAGE_IMAGE_DRAFT_ID_KEY].trim()
      : '';
  const conversationId =
    typeof imageElement.dataset[PAGE_IMAGE_CONVERSATION_ID_KEY] === 'string'
...
function getRenderableComparableSize(renderable) {
  const width =
    Number(renderable?.naturalWidth) || Number(renderable?.width) || 0;
  const height =
    Number(renderable?.naturalHeight) || Number(renderable?.height) || 0;
  if (width <= 0 || height <= 0) {
    return null;
  }
  return { width, height };
...
function shouldRejectPreviewOriginalBlobByAspectRatio(
  renderable,
  imageElement,
...
export function buildRecentImageSourceHint(
  imageElement,
  {
    now = Date.now(),
  ...
  const assetIds =
    typeof resolveAssetIds === 'function'
      ? resolveAssetIds(imageElement)
      : null;
  const sourceUrl =
    typeof resolveSourceUrl === 'function'
      ? String(resolveSourceUrl(imageElement) || '').trim()
      : '';
  const hasUsableSourceUrl =
    Boolean(sourceUrl) &&
    !isBlobPageImageSource(sourceUrl) &&
    !isDataPageImageSource(sourceUrl);
...
function isRecentImageSourceHintFresh(hint, now = Date.now()) {
  ...
  return (
    createdAt > 0 &&
    currentNow >= createdAt &&
    currentNow - createdAt <= RECENT_SOURCE_HINT_TTL_MS
  );
}

export function applyRecentImageSourceHintToImage(
  imageElement,
  hint,
  { now = Date.now() } = {},
) {
  if (
    !isRecentImageSourceHintFresh(hint, now) ||
    !imageElement ||
  ...
  if (
    ...
    !isDataPageImageSource(currentSourceUrl)
  ) {
    return false;
  }
  ...
  if (hasAnyAssetIds(imageAssetIds)) {
    if (
      !hasAnyAssetIds(hint.assetIds) ||
      !assetIdsMatch(imageAssetIds, hint.assetIds)
    ) {
      return false;
    }
  }
  ...
  if (
    hintSize &&
    imageSize &&
    getAspectRatioDelta(hintSize, imageSize) > 0.02
  ) {
    return false;
  }

  let applied = false;
  const rememberedSourceUrl =
    !hint.sourceUrl && hasAnyAssetIds(hint.assetIds)
      ? resolveRememberedOriginalAssetUrl(hint.assetIds)
      : '';
  const resolvedHintSourceUrl = hint.sourceUrl || rememberedSourceUrl;
  if (
    resolvedHintSourceUrl &&
    !(typeof dataset.gwrSourceUrl === 'string' && dataset.gwrSourceUrl.trim())
  ) {
  ...
  if (
    !dataset[PAGE_IMAGE_CONVERSATION_ID_KEY] &&
    hint.assetIds?.conversationId
  ) {
...
function getErrorCandidateDiagnostics(error) {
  return Array.isArray(error?.candidateDiagnostics)
    ? error.candidateDiagnostics
    : null;
}
...
function getDraftAssetRegistryKey(assetIds = null) {
  const draftId =
    typeof assetIds?.draftId === 'string' ? assetIds.draftId.trim() : '';
  return draftId ? `draft:${draftId}` : '';
}

function getResponseAssetRegistryKey(assetIds = null) {
  const responseId =
    typeof assetIds?.responseId === 'string' ? assetIds.responseId.trim() : '';
  const conversationId =
    typeof assetIds?.conversationId === 'string'
      ? assetIds.conversationId.trim()
      : '';
  return responseId && conversationId
    ? `response:${responseId}|conversation:${conversationId}`
    : '';
}

function rememberOriginalAssetUrlBinding(
  assetIds = null,
  sourceUrl = '',
...
function resolveRememberedPreviewSourceUrl(
  assetIds = null,
  { imageSessionStore = getDefaultImageSessionStore() } = {},
) {
  const sessionKey = imageSessionStore?.getOrCreateByAssetIds?.(assetIds) || '';
  ...
  const previewUrl =
    imageSessionStore?.getSnapshot?.(sessionKey)?.sources?.previewUrl || '';
  return typeof previewUrl === 'string' ? previewUrl.trim() : '';
}

function trimRememberedPreviewResultRegistry() {
  while (
    previewProcessedResultRegistry.size >
    MAX_REMEMBERED_PREVIEW_RESULT_REGISTRY_SIZE
  ) {
...
function rememberProcessedPreviewResult(
  sourceUrl = '',
  payload = {},
  { imageSessionStore = getDefaultImageSessionStore() } = {},
) {
  ...
  previewProcessedResultRegistry.set(normalizedSourceUrl, {
    ...
    processedFrom:
      typeof payload?.processedFrom === 'string' && payload.processedFrom.trim()
...
function resolveRememberedProcessedPreviewResult(
  sourceUrl = '',
  { imageSessionStore = getDefaultImageSessionStore() } = {},
) {
  const normalizedSourceUrl =
  ...
  const rememberedEntry =
    previewProcessedResultRegistry.get(normalizedSourceUrl) || null;
  if (!rememberedEntry?.sessionKey) {
    return null;
  }

  const rememberedResource =
    imageSessionStore?.getBestResource?.(
      rememberedEntry.sessionKey,
      'display',
    ) || null;
  if (
  ...
  return {
    ...
    processedMeta:
      rememberedResource.processedMeta ?? rememberedEntry.processedMeta ?? null,
    processedFrom:
      rememberedResource.source ||
      rememberedEntry.processedFrom ||
      'request-preview',
  };
}

function createPreviewCandidateProcessor(
  ...
  return new Promise((resolve, reject) => {
    ...
    const handleMessage = (event) => {
      ...
      if (event.data?.ok === false) {
        reject(
          new Error(
            normalizeErrorMessage(event.data?.error, 'Page fetch failed'),
          ),
        );
        return;
      }

      const blobMimeType = resolveFetchedImageMimeType(
        event.data?.mimeType,
        event.data?.buffer,
      );
...
export function intersectCaptureRectWithViewport(
  rect,
...
function readRenderedImageFitStyle(imageElement) {
  ...
  return {
    objectFit:
      typeof objectFit === 'string' ? objectFit.trim().toLowerCase() : '',
    objectPosition:
      typeof objectPosition === 'string'
        ? objectPosition.trim().toLowerCase()
        : '',
  };
}
...
function resolveRenderedImageContentRect(imageElement, imageRect) {
  ...
  const remainingHorizontalSpace = Math.max(
    0,
    normalizedImageRect.width - renderedWidth,
  );
  const remainingVerticalSpace = Math.max(
    0,
    normalizedImageRect.height - renderedHeight,
  );
  const [xToken = '50%', yToken = '50%'] = objectPosition
    .split(/\s+/)
    .filter(Boolean);
...
export function resolveVisibleCaptureRect(imageElement) {
  const imageRect = normalizeCaptureRect(
    imageElement?.getBoundingClientRect?.(),
  );
  const imageContentRect = resolveRenderedImageContentRect(
    imageElement,
    imageRect,
  );
...
function isPreviewImageRenderable(imageElement) {
  return (
    Boolean(imageElement?.complete) &&
    (Number(imageElement?.naturalWidth) || 0) > 0 &&
    (Number(imageElement?.naturalHeight) || 0) > 0
  );
}

export async function waitForRenderableImageSize(
  imageElement,
  timeoutMs = 1500,
) {
...
function hasConfirmedGeminiPreviewMeta(processedMeta) {
  return (
    classifyGeminiAttributionFromWatermarkMeta(processedMeta).tier !==
    'insufficient'
  );
}
...
function summarizeCandidateDiagnostics(diagnostics) {
  ...
  return diagnostics
    .map((item) => {
      ...
      if (typeof item.sourceBlobSize === 'number')
        parts.push(`sourceSize=${item.sourceBlobSize}`);
      if (item.sourceBlobType) parts.push(`sourceType=${item.sourceBlobType}`);
      if (typeof item.processedBlobSize === 'number')
        parts.push(`processedSize=${item.processedBlobSize}`);
      if (item.processedBlobType)
        parts.push(`processedType=${item.processedBlobType}`);
...
export function shouldSkipPreviewProcessingFailure(diagnostics = []) {
  ...
  const pageFetchFailure = diagnostics.find(
    (item) => item?.strategy === 'page-fetch',
  );
  const renderedCaptureFailure = diagnostics.find(
    (item) => item?.strategy === 'rendered-capture',
  );
  const pageFetchError = String(pageFetchFailure?.error || '');
  const renderedCaptureError = String(renderedCaptureFailure?.error || '');

  return (
    pageFetchFailure?.status === 'error' &&
...
export async function resolvePreviewReplacementResult({
  ...
  for (const candidate of candidates) {
    try {
      ...
      const decisionTier =
        classifyGeminiAttributionFromWatermarkMeta(result?.processedMeta)
          .tier || 'insufficient';
      diagnostics.push({
        ...
        processorPath:
          typeof result?.processedMeta?.processorPath === 'string'
            ? result.processedMeta.processorPath
            : '',
        sourceBlobType: result?.sourceBlobType || '',
        sourceBlobSize:
          typeof result?.sourceBlobSize === 'number'
            ? result.sourceBlobSize
            : undefined,
        processedBlobType: result?.processedBlob?.type || '',
        processedBlobSize:
          typeof result?.processedBlob?.size === 'number'
            ? result.processedBlob.size
            : undefined,
      });
      if (confirmed) {
        return {
      ...
      if (
        isSafePreviewFallbackStrategy(candidate.strategy) &&
        isBlobLike(result?.processedBlob)
      ) {
      ...
      diagnostics.push({
        ...
        status: 'error',
        sourceBlobType:
          typeof error?.sourceBlobType === 'string' ? error.sourceBlobType : '',
        sourceBlobSize:
          typeof error?.sourceBlobSize === 'number'
            ? error.sourceBlobSize
            : undefined,
  ...
  if (lastError) {
    const wrappedError = new Error(
      normalizeErrorMessage(lastError, 'Preview candidate failed'),
    );
    wrappedError.candidateDiagnostics = diagnostics;
    wrappedError.candidateDiagnosticsSummary =
      summarizeCandidateDiagnostics(diagnostics);
    throw wrappedError;
  }
...
export async function processOriginalPageImageSource({
  ...
  const validateOriginalBlob =
    typeof validateBlob === 'function'
      ? async (blob) => {
  ...
  if (
    rejectPreviewAspectMismatch &&
    shouldRejectPreviewOriginalBlobByAspectRatio(
      validatedRenderable,
      imageElement,
    )
  ) {
...
export async function processPageImageSource({
  ...
  const treatAsPreviewSource = shouldTreatPageImageSourceAsPreview(
    imageElement,
    sourceUrl,
  );
  if (treatAsPreviewSource) {
    ...
    return processPreviewPageImageSource({
      sourceUrl,
      imageElement,
      fetchPreviewBlob: isBlobPageImageSource(sourceUrl)
        ? null
        : fetchPreviewBlob,
      processWatermarkBlobImpl,
      captureRenderedImageBlob,
  ...
  return processOriginalPageImageSource({
    ...
    preferRenderedCaptureForPreview:
      isGeminiDisplayPreviewAssetUrl(sourceUrl) &&
      !hasExplicitBoundSourceUrl(imageElement, sourceUrl),
  });
}
...
function hasRelevantDescendant(root) {
  ...
  return Boolean(
    root.querySelector('img') && root.querySelector('button,[role="button"]'),
  );
}
...
export function shouldScheduleMutationRoot(root) {
  ...
  const tagName =
    typeof root.tagName === 'string' ? root.tagName.toUpperCase() : '';
  if (!tagName) {
    return false;
  }
...
export function shouldScheduleAttributeMutation(target, attributeName = '') {
  ...
  const normalizedAttributeName =
    typeof attributeName === 'string' ? attributeName.trim().toLowerCase() : '';
  ...
  if (
    ...
    normalizedAttributeName !== 'srcset'
  ) {
    return true;
  }
...
export function isSelfWrittenProcessedImageSource(target) {
  const trackedObjectUrl =
    typeof target?.dataset?.[PAGE_IMAGE_OBJECT_URL_KEY] === 'string'
...
export function handlePageImageMutations(
  mutations,
  { scheduleProcess, HTMLImageElementClass = globalThis.HTMLImageElement } = {},
) {
  if (
    typeof scheduleProcess !== 'function' ||
  ...
  for (const mutation of mutations) {
    if (mutation?.type === 'attributes') {
      if (
        !hasImageElementClass ||
        !(mutation.target instanceof HTMLImageElementClass)
      ) {
        continue;
      }
      if (
...
export function showProcessingOverlay(
  imageElement,
  {
    container = getPreferredGeminiImageContainer(imageElement) ||
  ...
  if (existingState) {
    ...
    if (
      existingState.overlay?.style &&
      typeof existingState.overlay.style === 'object'
    ) {
  ...
  const previousFilter =
    typeof imageElement?.style?.filter === 'string'
      ? imageElement.style.filter
      : '';
  const previousContainerPosition =
    typeof container?.style?.position === 'string'
...
function revokeTrackedObjectUrl(imageElement) {
  ...
  if (
    previewState?.overlay?.parentNode &&
    typeof previewState.overlay.parentNode.removeChild === 'function'
  ) {
    previewState.overlay.parentNode.removeChild(previewState.overlay);
  }
  if (
    previewState?.overlay?.style &&
    typeof previewState.overlay.style === 'object'
  ) {
...
function resolvePreviewOverlayMount(imageElement) {
  const container =
    getPreferredGeminiImageContainer(imageElement) ||
    imageElement?.parentElement ||
    null;
  if (!container) {
    return {
  ...
  const controlsParent =
    controls?.parentElement || controls?.parentNode || null;
...
function applyReadyImageState(
  ...
  const assetIds =
    readAssetIdsFromImageDataset(imageElement) ||
...
export function preparePageImageProcessing(
  imageElement,
  {
    processing = null,
  ...
  const isHtmlImageElement =
    typeof HTMLImageElementClass === 'function' &&
    imageElement instanceof HTMLImageElementClass;
  const isImageLikeElement =
    typeof imageElement?.tagName === 'string' &&
    imageElement.tagName.toUpperCase() === 'IMG';
  if (!isHtmlImageElement && !isImageLikeElement) {
    return null;
  }
  if (
    typeof isProcessableImage === 'function' &&
    !isProcessableImage(imageElement)
  ) {
    return null;
  }
  ...
  const rememberedPreviewSourceUrl =
    isBlobPageImageSource(sourceUrl) || isDataPageImageSource(sourceUrl)
      ? resolveRememberedPreviewSourceUrl(assetIds, { imageSessionStore })
      : '';
  const rememberedBoundSourceUrl =
    rememberedSourceUrl || rememberedPreviewSourceUrl;
  if (
    rememberedBoundSourceUrl &&
    (!dataset.gwrSourceUrl ||
      isBlobPageImageSource(sourceUrl) ||
      isDataPageImageSource(sourceUrl))
  ) {
  ...
  const sessionKey = imageSessionStore?.getOrCreateByAssetIds?.(assetIds) || '';
  const isPreviewSource = shouldTreatPageImageSourceAsPreview(
    imageElement,
    sourceUrl,
  );
  if (sessionKey) {
    ...
    if (isPreviewSource) {
      const existingPreviewResource =
        imageSessionStore.getBestResource?.(sessionKey, 'display') || null;
      if (
...
export function applyPageImageProcessingResult({
  ...
  if (sourceResult?.skipped) {
    ...
    emitPageImageProcessEvent({
      ...
      payload: {
        ...
        candidateDiagnosticsSummary:
          sourceResult.candidateDiagnosticsSummary || '',
      },
    });
    return;
  }
  ...
  const candidateDiagnosticsSummary =
    sourceResult?.candidateDiagnosticsSummary || '';
  ...
  applyReadyImageState(imageElement, processedBlob, {
    ...
    processedFrom:
      selectedStrategy || (isPreviewSource ? 'preview-candidate' : 'default'),
...
export function bindOriginalAssetUrlToImages({
  ...
  for (const imageElement of collectBindableImages(root)) {
    const imageAssetIds = {
      ...
      conversationId:
        imageElement?.dataset?.[PAGE_IMAGE_CONVERSATION_ID_KEY] || null,
    };
    const resolvedImageAssetIds =
      imageAssetIds.responseId ||
      imageAssetIds.draftId ||
      imageAssetIds.conversationId
        ? imageAssetIds
        : extractGeminiImageAssetIds(imageElement);
    ...
    if (dataset.gwrSourceUrl === normalizedSourceUrl) {
      const rememberedPreviewResult = resolveRememberedProcessedPreviewResult(
        normalizedSourceUrl,
        {
          imageSessionStore,
        },
      ...
      if (rememberedPreviewResult) {
        applyReadyImageState(
          imageElement,
          rememberedPreviewResult.processedBlob,
          {
            imageSessionStore,
...
export function bindProcessedPreviewResultToImages({
  ...
  let updatedCount = 0;
  let rememberedSessionKey =
    typeof sessionKey === 'string' ? sessionKey.trim() : '';
  for (const imageElement of collectBindableImages(root)) {
    const candidateSourceUrl = normalizeGoogleusercontentImageUrl(
      resolveCandidateImageUrl(imageElement) || '',
    );
    ...
    if (!rememberedSessionKey) {
      const rememberedAssetIds =
        readAssetIdsFromImageDataset(imageElement) ||
        extractGeminiImageAssetIds(imageElement);
      rememberedSessionKey =
        imageSessionStore.getOrCreateByAssetIds?.(rememberedAssetIds) || '';
    }
    updatedCount += 1;
  }

  rememberProcessedPreviewResult(
    normalizedSourceUrl,
    {
...
export function createPageImageReplacementController({
  ...
  function tryApplyRememberedPreviewResult(imageElement) {
    ...
    const datasetSourceUrl =
      typeof imageElement?.dataset?.gwrSourceUrl === 'string'
        ? imageElement.dataset.gwrSourceUrl.trim()
        : '';
    const stableSourceUrl =
      typeof imageElement?.dataset?.gwrStableSource === 'string'
  ...
  function tryApplySessionProcessedResult(imageElement) {
    ...
    const resource =
      imageSessionStore?.getBestResource?.(sessionKey, 'display') || null;
    if (resource?.kind !== 'processed' || !(resource.blob instanceof Blob)) {
      return false;
    }
  ...
  async function processImage(imageElement) {
    ...
    try {
      const sourceResult = await processPageImageSourceImpl(
