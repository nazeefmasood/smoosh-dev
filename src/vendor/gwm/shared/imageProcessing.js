...
function loadImageFromObjectUrl(objectUrl) {
  return new Promise((resolve, reject) => {
    ...
    image.onerror = () =>
      reject(new Error('Failed to decode Gemini image blob'));
    image.src = objectUrl;
  });
}
...
function normalizeProcessorResult(result, processorPath = 'main-thread') {
  return {
    ...
    processedMeta: withProcessorPath(
      result?.processedMeta || null,
      processorPath,
    ),
  };
}
...
export function createSharedBlobProcessor({
  ...
  return async function processWithBestPath(
    blob,
    options = { adaptiveMode: 'always' },
  ) {
    const normalizedOptions = normalizeProcessingOptions(options);
    const processWorker =
      typeof getWorkerProcessor === 'function' ? getWorkerProcessor() : null;
...
export async function processWatermarkBlob(
  blob,
  options = { adaptiveMode: 'always' },
) {
...
export async function removeWatermarkFromBlob(
  blob,
  options = { adaptiveMode: 'always' },
) {
