...
async function captureRenderedBlob({ image, captureRenderedImageBlob }) {
...
export async function acquireOriginalBlob({
  ...
  if (
    ...
    shouldPreferRenderedCapture(normalizedSourceUrl)
  ) {
  ...
  if (
    preferRenderedCaptureForBlobUrl &&
    isRuntimeBlobUrl(normalizedSourceUrl)
  ) {
    return captureRenderedBlob({
      image,
      captureRenderedImageBlob,
