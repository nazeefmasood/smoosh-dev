...
function normalizeSurfaceType(surface = '') {
  const normalizedSurface =
    typeof surface === 'string' ? surface.trim().toLowerCase() : '';
...
function readElementProcessedObjectUrl(element) {
  const objectUrl =
    typeof element?.dataset?.gwrWatermarkObjectUrl === 'string'
...
function findPreferredSurfaceElement(elements, preferredProcessedUrl = '') {
  ...
  for (const element of elements) {
    ...
    if (
      ...
      processedObjectUrl === preferredProcessedUrl
    ) {
      return element;
    }
...
export function createImageSessionStore({ now = () => Date.now() } = {}) {
  ...
  function getOrCreateByAssetIds(assetIds = null) {
    ...
    if (!session.assetIds) {
      ...
      session.assetIds = {
        responseId:
          session.assetIds.responseId || normalizedAssetIds.responseId,
        draftId: session.assetIds.draftId || normalizedAssetIds.draftId,
        conversationId:
          session.assetIds.conversationId || normalizedAssetIds.conversationId,
      };
    }
  ...
  function updateSourceSnapshot(
    sessionKey,
    { sourceUrl = '', isPreviewSource = false } = {},
  ) {
    const session = getSession(sessionKey);
    const normalizedUrl = typeof sourceUrl === 'string' ? sourceUrl.trim() : '';
    if (!session || !normalizedUrl) {
  ...
  function updateProcessedResult(
    sessionKey,
    {
      slot = 'preview',
      objectUrl = '',
    ...
    const normalizedObjectUrl =
      typeof objectUrl === 'string' ? objectUrl.trim() : '';
    ...
    session.derived.processedSlots[normalizedSlot] = {
      ...
      processedFrom:
        typeof processedFrom === 'string' ? processedFrom.trim() : '',
    };
  ...
  function markProcessing(sessionKey, surface, status, error = '') {
    ...
    const normalizedSurface = normalizeSurfaceType(surface);
    session.state[normalizedSurface] =
      typeof status === 'string' ? status : 'idle';
  ...
  function getBestResource(sessionKey, action = 'display') {
    ...
    const previewProcessedResource = readProcessedSlotResource(
      session,
      'preview',
    );
  ...
  function getPreferredElement(sessionKey, action = 'display') {
    ...
    const preferredProcessedUrl =
      preferredResource?.kind === 'processed'
