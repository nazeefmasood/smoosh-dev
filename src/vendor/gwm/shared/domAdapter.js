...
const GEMINI_IMAGE_CONTAINER_SELECTOR =
  'generated-image,.generated-image-container';
const GEMINI_FULLSCREEN_CONTAINER_SELECTOR =
  'expansion-dialog,[role="dialog"],.image-expansion-dialog-panel,.cdk-overlay-pane';
const GEMINI_UPLOADER_PREVIEW_SELECTOR =
  '[data-test-id="image-preview"],uploader-file-preview,uploader-file-preview-container,.attachment-preview-wrapper,.file-preview-container';
...
function parseGeminiAssetIdsFromJslog(jslog = '') {
  ...
  const responseId = normalizeGeminiAssetId(
    jslog.match(/"((?:r|resp)_[^"]+)"/)?.[1] || null,
    'r_',
  );
  const conversationId = normalizeGeminiAssetId(
    jslog.match(/"((?:c|conv)_[^"]+)"/)?.[1] || null,
    'c_',
  );
  const draftId = normalizeGeminiAssetId(
    jslog.match(/"((?:rc|draft)_[^"]+)"/)?.[1] || null,
    'rc_',
  );
...
export function resolveCandidateImageUrl(img) {
  ...
  const explicitSource =
    typeof img?.dataset?.gwrSourceUrl === 'string'
      ? img.dataset.gwrSourceUrl.trim()
      : '';
  if (explicitSource) return explicitSource;
  const stableSource =
    typeof img?.dataset?.gwrStableSource === 'string'
      ? img.dataset.gwrStableSource.trim()
      : '';
  if (stableSource) {
    const currentSrc =
...
export function getGeminiImageQuerySelector() {
  return GEMINI_IMAGE_CONTAINER_SELECTOR.split(',')
...
export function extractGeminiImageAssetIds(img) {
  ...
  const responseIdFromDataset = normalizeGeminiAssetId(
    typeof img?.dataset?.gwrResponseId === 'string'
      ? img.dataset.gwrResponseId
      : null,
    'r_',
  );
  ...
  const draftIdFromDataset = normalizeGeminiAssetId(
    typeof img?.dataset?.gwrDraftId === 'string'
      ? img.dataset.gwrDraftId
      : null,
    'rc_',
  );
  ...
  const conversationIdFromDataset = normalizeGeminiAssetId(
    typeof img?.dataset?.gwrConversationId === 'string'
      ? img.dataset.gwrConversationId
      : null,
    'c_',
  );
  ...
  for (const element of collectGeminiMetadataElements(img)) {
    ...
    const parsed = parseGeminiAssetIdsFromJslog(
      getAttributeValue(element, 'jslog'),
    );
    if (!parsed) continue;
