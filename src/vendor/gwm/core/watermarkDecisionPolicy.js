...
export function classifyStandardWatermarkSignal({
  spatialScore,
  gradientScore,
}) {
...
export function classifyGeminiAttributionFromWatermarkMeta(watermarkMeta) {
  ...
  if (
    size === null ||
    size < ATTRIBUTION_MIN_SIZE ||
    size > ATTRIBUTION_MAX_SIZE
  ) {
    return { tier: 'insufficient' };
  }
  ...
  const source =
    typeof watermarkMeta.source === 'string' ? watermarkMeta.source : '';
