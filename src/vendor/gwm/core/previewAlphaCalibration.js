...
export function estimatePreviewAlphaMap({
  ...
  if (!sourceImageData || !previewImageData || !position) {
    throw new TypeError(
      'estimatePreviewAlphaMap requires sourceImageData, previewImageData, and position',
    );
  }
  if (
    sourceImageData.width !== previewImageData.width ||
    sourceImageData.height !== previewImageData.height
  ) {
    throw new RangeError(
      'sourceImageData and previewImageData must have identical dimensions',
    );
  }
  ...
  for (let row = 0; row < height; row++) {
    for (let col = 0; col < width; col++) {
      ...
      const r = resolveChannelAlpha(
        sourceImageData.data[idx],
        previewImageData.data[idx],
      );
      const g = resolveChannelAlpha(
        sourceImageData.data[idx + 1],
        previewImageData.data[idx + 1],
      );
      const b = resolveChannelAlpha(
        sourceImageData.data[idx + 2],
        previewImageData.data[idx + 2],
      );
...
export function aggregatePreviewAlphaMaps(alphaMaps) {
  if (!Array.isArray(alphaMaps) || alphaMaps.length === 0) {
    throw new TypeError(
      'aggregatePreviewAlphaMaps requires at least one alpha map',
    );
  }
  ...
  if (!Number.isInteger(expectedLength) || expectedLength <= 0) {
    throw new TypeError(
      'alpha maps must be typed arrays with a positive length',
    );
  }
  ...
  for (let row = 0; row < position.height; row++) {
    for (let col = 0; col < position.width; col++) {
      ...
      for (let channel = 0; channel < 3; channel++) {
        ...
        imageData.data[idx + channel] = clampChannel(
          alpha * 255 + (1 - alpha) * original,
        );
      }
    }
  }
    ...
    for (let col = 0; col < position.width; col++) {
      ...
      for (let dy = -1; dy <= 1; dy++) {
        for (let dx = -1; dx <= 1; dx++) {
          ...
          const idx =
            ((position.y + localY) * imageData.width + (position.x + localX)) *
            4;
          const w = dx === 0 && dy === 0 ? 4 : dx === 0 || dy === 0 ? 2 : 1;
      ...
      const outIdx =
        ((position.y + row) * blurred.width + (position.x + col)) * 4;
...
function measureRegionAbsDelta(candidateImageData, targetImageData, position) {
  ...
  for (let row = 0; row < position.height; row++) {
    for (let col = 0; col < position.width; col++) {
      const idx =
        ((position.y + row) * candidateImageData.width + (position.x + col)) *
        4;
      for (let channel = 0; channel < 3; channel++) {
        total += Math.abs(
          candidateImageData.data[idx + channel] -
            targetImageData.data[idx + channel],
        );
        count++;
      }
    }
...
export function measurePreviewBoundaryMetrics(
  candidateImageData,
  previewImageData,
  position,
) {
  ...
  const compareBoundaryPixel = (x, y, insideSamples, outsideSamples) => {
    ...
    for (let channel = 0; channel < 3; channel++) {
      rawTotal += Math.abs(
        candidateImageData.data[idx + channel] - outside[channel],
      );
      previewBoundaryTotal += Math.abs(
        previewImageData.data[previewIdx + channel] - outside[channel],
      );
...
function measurePreviewBoundaryContinuity(
  candidateImageData,
  previewImageData,
  position,
) {
  return measurePreviewBoundaryMetrics(
    candidateImageData,
    previewImageData,
    position,
  ).normalizedScore;
...
export function renderPreviewWatermarkObservation({
  ...
  if (!sourceImageData || !alphaMap || !position) {
    throw new TypeError(
      'renderPreviewWatermarkObservation requires sourceImageData, alphaMap, and position',
    );
  }
...
export function fitConstrainedPreviewAlphaModel({
  ...
  if (!sourceImageData || !previewImageData || !standardAlphaMap || !position) {
    throw new TypeError(
      'fitConstrainedPreviewAlphaModel requires sourceImageData, previewImageData, standardAlphaMap, and position',
    );
  }
  ...
  if (
    ...
    standardAlphaMap.length !== size * size
  ) {
    throw new RangeError(
      'fitConstrainedPreviewAlphaModel requires a square ROI and matching standardAlphaMap size',
    );
  }
      ...
      for (const dx of shiftCandidates) {
        ...
        for (const blurRadius of blurRadii) {
          ...
          for (const alphaGain of alphaGainCandidates) {
            ...
            const score = measureRegionAbsDelta(
              restored,
              sourceImageData,
              position,
            );
...
export function fitPreviewRenderModel({
  ...
  if (!sourceImageData || !previewImageData || !standardAlphaMap || !position) {
    throw new TypeError(
      'fitPreviewRenderModel requires sourceImageData, previewImageData, standardAlphaMap, and position',
    );
  }
  ...
  if (
    ...
    throw new RangeError(
      'fitPreviewRenderModel requires a square ROI and matching standardAlphaMap size',
    );
  }
...
export function fitPreviewOnlyRenderModel({
  ...
  if (!previewImageData || !standardAlphaMap || !position) {
    throw new TypeError(
      'fitPreviewOnlyRenderModel requires previewImageData, standardAlphaMap, and position',
    );
  }
  ...
  if (
    ...
    throw new RangeError(
      'fitPreviewOnlyRenderModel requires a square ROI and matching standardAlphaMap size',
    );
  }

  const resolvedPriorRadiusCandidates =
    Array.isArray(priorRadiusCandidates) && priorRadiusCandidates.length > 0
  ...
  for (const candidatePriorRadius of resolvedPriorRadiusCandidates) {
    ...
    for (const scale of scaleCandidates) {
      for (const dy of shiftCandidates) {
        for (const dx of shiftCandidates) {
          const warped = warpAlphaMap(standardAlphaMap, size, {
            dx,
            dy,
            scale,
          });
          for (const alphaBlurRadius of alphaBlurRadii) {
            ...
            for (const compositeBlurRadius of compositeBlurRadii) {
              for (const alphaGain of alphaGainCandidates) {
                ...
                const forwardScore = measureRegionAbsDelta(
                  rendered,
                  previewImageData,
                  position,
                );
                ...
                if (
                  !alphaBestForRadius ||
                  forwardScore < alphaBestForRadius.forwardScore
                ) {
                  alphaBestForRadius = {
                    alphaMap,
                    alphaGain,
  ...
  for (const alphaCandidate of alphaCandidates) {
    for (const blendStrength of blendStrengthCandidates) {
      ...
      const inverseScore = measureRegionAbsDelta(
        rerendered,
        previewImageData,
        position,
      );
...
export function restorePreviewRegionWithRenderModel({
  ...
  if (!previewImageData || !alphaMap || !position) {
    throw new TypeError(
      'restorePreviewRegionWithRenderModel requires previewImageData, alphaMap, and position',
    );
  }
  ...
  if (compositeBlurRadius > 0 && totalIterations > 0) {
    for (let iteration = 0; iteration < totalIterations; iteration++) {
      const reblurred = blurImageDataRegion(
        deblurred,
        position,
        compositeBlurRadius,
      );
      ...
      for (let row = 0; row < position.height; row++) {
        for (let col = 0; col < position.width; col++) {
          const idx =
            ((position.y + row) * deblurred.width + (position.x + col)) * 4;
          for (let channel = 0; channel < 3; channel++) {
            const error =
              previewImageData.data[idx + channel] -
              reblurred.data[idx + channel];
...
export function restorePreviewRegionWithNeighborhoodPrior({
  ...
  if (!previewImageData || !alphaMap || !position || !priorImageData) {
    throw new TypeError(
      'restorePreviewRegionWithNeighborhoodPrior requires previewImageData, alphaMap, position, and priorImageData',
    );
  }
  ...
  const resolvedBlendStrength = Number.isFinite(blendStrength)
    ? blendStrength
    : 0.85;
  for (let row = 0; row < position.height; row++) {
    for (let col = 0; col < position.width; col++) {
      ...
      const blend = Math.max(
        0,
        Math.min(1, Math.sqrt(alpha) * resolvedBlendStrength),
      );
      const idx =
        ((position.y + row) * restored.width + (position.x + col)) * 4;
      for (let channel = 0; channel < 3; channel++) {
