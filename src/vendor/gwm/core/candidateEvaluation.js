...
export function hasSafeDefaultAlphaNewMarginResidual(candidate) {
  ...
  return (
    Math.abs(numberOr(candidate?.processedSpatialScore, Infinity)) <=
      DEFAULT_ALPHA_NEW_MARGIN_MAX_SPATIAL_RESIDUAL &&
    Math.max(0, numberOr(candidate?.processedGradientScore, Infinity)) <=
      DEFAULT_ALPHA_NEW_MARGIN_MAX_GRADIENT_RESIDUAL &&
    numberOr(candidate?.improvement, -Infinity) >=
...
export function hasHighRiskNewMarginPositiveEvidence(candidate) {
  ...
  return (
    numberOr(candidate?.originalGradientScore) >=
      HIGH_RISK_NEW_MARGIN_MIN_GRADIENT ||
    numberOr(candidate?.originalSpatialScore) >=
...
export function shouldPreferDefaultAlphaNewMarginCandidate(
  currentBest,
  candidate,
) {
