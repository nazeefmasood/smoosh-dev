...
export function createAcceptedDecisionPath({
  ...
  const evaluation = {
    ...
    finalScores: {
      originalSpatial: finiteOrNull(
        originalSpatialScore ?? selectedTrial?.originalSpatialScore,
      ),
      originalGradient: finiteOrNull(
...
export function createDecisionPathContractSummary(decisionPath = null) {
  return {
    ...
    hasDetectionCandidate:
      decisionPath?.detectionCandidate !== null &&
      decisionPath?.detectionCandidate !== undefined,
    hasAlphaTrial:
      decisionPath?.alphaTrial !== null &&
      decisionPath?.alphaTrial !== undefined,
    hasRepairTrial:
      decisionPath?.repairTrial !== null &&
      decisionPath?.repairTrial !== undefined,
  };
}
