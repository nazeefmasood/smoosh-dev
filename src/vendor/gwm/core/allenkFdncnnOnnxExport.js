...
function readFloat32LE(bytes, offset) {
  ...
  return new DataView(buffer.buffer, buffer.byteOffset + offset, 4).getFloat32(
    0,
    true,
  );
}
...
function createShapeProto(dims) {
  return message(
    dims.map((dim) => messageField(1, message([int64Field(1, dim)]))),
  );
}
...
function createValueInfoProto({ name, dims }) {
  ...
  return message([stringField(1, name), messageField(2, typeProto)]);
}
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    nodes.push(
      createNodeProto({
        name: `conv${index}`,
        opType: 'Conv',
        inputs: [previous, `conv${index}.weight`, `conv${index}.bias`],
        outputs: [convOutput],
        attributes: [
          createIntsAttribute('kernel_shape', [
            segment.kernelH,
            segment.kernelW,
          ]),
          createIntsAttribute('pads', [
            segment.padH,
            segment.padW,
            segment.padH,
            segment.padW,
...
function createGraphProto({
  ...
  const outputShape = [
    1,
    segments[segments.length - 1].outputChannels,
    roiHeight,
    roiWidth,
  ];
  ...
  return {
    ...
    bytes: message([
      ...
      messageField(
        11,
        createValueInfoProto({ name: inputName, dims: inputShape }),
      ),
      messageField(
        12,
        createValueInfoProto({ name: outputName, dims: outputShape }),
      ),
    ]),
  };
}
...
function exportAllenkFdncnnOnnx({
  ...
  return {
    ...
    metadata: {
      ...
      reluNodeCount: segments.filter(
        (segment, index) =>
          index < segments.length - 1 && segment.activationType === 1,
      ).length,
