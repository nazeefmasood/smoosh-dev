...
import { calculateNearBlackRatio, scoreRegion } from './candidateSelector.js';
import {
  assessAlphaBandHalo,
...
import { createRegionCorrelationMetrics } from './pipelineMetrics.js';
...
const ALPHA_GAIN_CANDIDATES = ALPHA_PARAMETER_GROUPS.map(
  (group) => group.alphaGain,
);
const LOCATED_AGGRESSIVE_ALPHA_GAINS = Object.freeze([
...
const CATALOG_DARK_ALPHA_GAIN_CANDIDATES = Object.freeze([
  0.9, 0.85, 0.8, 0.95, 0.7, 0.6,
]);
const STANDARD_ALPHA_PRIORITY_GAINS = ALPHA_PARAMETER_GROUPS.filter(
...
const KNOWN_48_ANTI_TEMPLATE_RESCUE_GAINS = Object.freeze([
...
const KNOWN_48_RESIDUAL_REBALANCE_GAINS = Object.freeze([
...
function refineSubpixelOutline({
  ...
  const maxAllowedNearBlackRatio = Math.min(
    1,
    originalNearBlackRatio + MAX_NEAR_BLACK_RATIO_INCREASE,
  );
...
function applyLumaEdgeCorrection({
  ...
  for (let row = 0; row < size; row++) {
    for (let col = 0; col < size; col++) {
      ...
      candidate.data[pixelIndex + 1] = clampChannel(
        data[pixelIndex + 1] + finalDelta,
      );
      candidate.data[pixelIndex + 2] = clampChannel(
...
function applyMidCoreBiasCorrection({
  ...
  for (let row = 0; row < position.height; row++) {
    for (let col = 0; col < position.width; col++) {
      ...
      candidate.data[pixelIndex] = clampChannel(
  ...
  for (let row = 0; row < position.height; row++) {
    for (let col = 0; col < position.width; col++) {
      ...
      alphaMap[localIndex] = Math.min(
        0.9,
        clamp01((estimatedAlpha - threshold) * 1.2),
      );
    }
  }
...
function applyPreviewSmoothBackgroundCleanup({ imageData, position }) {
  ...
  for (let row = 0; row < expandedPosition.height; row++) {
    ...
    for (let col = 0; col < expandedPosition.width; col++) {
      ...
      candidate.data[idx + 1] = clampChannel(
        (horizontal[1] + vertical[1]) * 0.5,
      );
      candidate.data[idx + 2] = clampChannel(
...
function refinePreviewResidualEdge({
  ...
  const maxAllowedNearBlackRatio = Math.min(
    1,
    baselineNearBlackRatio + MAX_NEAR_BLACK_RATIO_INCREASE,
  );
