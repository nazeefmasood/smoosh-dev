...
function readInt32LE(buffer, offset) {
  ...
  return buffer.readInt32LE
    ...
    : new DataView(buffer.buffer, buffer.byteOffset + offset, 4).getInt32(
        0,
        true,
      );
}
...
function readUInt32LE(buffer, offset) {
  ...
  return buffer.readUInt32LE
    ...
    : new DataView(buffer.buffer, buffer.byteOffset + offset, 4).getUint32(
        0,
        true,
      );
}
...
function buildAllenkFdncnnWeightLayout(param, binLike) {
  ...
  for (const layer of param.layers) {
    ...
    const expectedWeightCount =
      conv.numOutput * inputChannels * conv.kernelW * conv.kernelH;
    ...
    if (storageTag !== NCNN_WEIGHT_FP16_STORAGE_TAG) {
      throw new Error(
        `Unexpected NCNN weight storage tag at layer ${
          layer.index
        }: 0x${storageTag.toString(16)}`,
      );
    }
...
function summarizeAllenkFdncnnModel(param, weightLayout = null) {
  const convolutionLayers = param.layers.filter(
    (layer) => layer.type === 'Convolution',
  );
  const reluLayers = convolutionLayers.filter(
    (layer) => layer.convolution?.activationType === 1,
  );
