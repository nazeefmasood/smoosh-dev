...
function calculateLayoutMacs(segments, width, height) {
  return segments.reduce(
    (sum, segment) => sum + calculateSegmentMacs(segment, width, height),
    0,
  );
}
...
function decodeSegmentWeights(segment, weightBin) {
  ...
  for (let i = 0; i < segment.weightCount; i++) {
    weights[i] = halfToFloat(
      readUInt16LE(weightBin, segment.weightOffset + i * 2),
    );
  }
        ...
        for (let ic = 0; ic < inputChannels; ic++) {
          ...
          for (let ky = 0; ky < kernelH; ky++) {
            ...
            for (let kx = 0; kx < kernelW; kx++) {
              ...
              sum +=
                input[inputBase + sy * width + sx] *
                weights[weightBase + ky * kernelW + kx];
            }
          }
        }
...
function createAllenkFdncnnReferenceRuntime({
  ...
  const bin =
    weightBin instanceof Uint8Array ? weightBin : new Uint8Array(weightBin);
    ...
    execute(input, width, height) {
      ...
      if (macs > maxMacs) {
        const error = new Error(
          `allenk FDnCNN pure JS reference refused ${macs} MACs; max=${maxMacs}`,
        );
