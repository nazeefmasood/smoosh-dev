...
export function removeRepeatedWatermarkLayers(
  ...
  const imageData = isObjectCall
    ? imageDataOrOptions.imageData
    : imageDataOrOptions;
  ...
  const residualThreshold =
    options.residualThreshold ?? DEFAULT_RESIDUAL_THRESHOLD;
  ...
  const maxNearBlackRatio = Math.min(
    1,
    baseNearBlackRatio + MAX_NEAR_BLACK_RATIO_INCREASE,
  );
  const passes = [];
  ...
  for (let passIndex = 0; passIndex < maxPasses; passIndex++) {
    ...
    const improvement =
      Math.abs(before.spatialScore) - Math.abs(after.spatialScore);
