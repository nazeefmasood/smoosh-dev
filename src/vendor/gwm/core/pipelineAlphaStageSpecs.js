...
export function createRecalibrationStageSpec({
  ...
  const { recalibrateAlphaStrength } = refiners;
  const state = readPipelineAlphaState(readState);

  return {
    shouldRun:
      typeof shouldRecalibrateAlphaStrength === 'function'
        ? shouldRecalibrateAlphaStrength({
  ...
  return [
    {
      ...
      createStage: () => {
        ...
        const refined =
          ...
          typeof refineSmallPreviewAnchorCandidate === 'function'
            ? refineSmallPreviewAnchorCandidate({
                originalImageData,
                source: state.source,
                position: state.position,
    ...
    },
    {
      ...
      createStage: () => {
        ...
        const currentResidualVisibility =
          typeof assessWatermarkResidualVisibility === 'function'
            ? assessWatermarkResidualVisibility({
...
export function createSubpixelOutlineAlphaStageSpecs({
  ...
  const { refineSubpixelOutline } = refiners;
  const {
...
export function createLocatedAggressiveStageSpec({
  ...
  const { refineLocatedAggressiveRemoval } = refiners;
  const state = readPipelineAlphaState(readState);
  ...
  const skipCleanCanonical96 =
    baseShouldRun &&
    typeof shouldSkipLocatedAggressiveForCleanCanonical96 === 'function'
      ? shouldSkipLocatedAggressiveForCleanCanonical96({
          config: state.config,
          alphaGain: state.alphaGain,
          originalSpatialScore,
