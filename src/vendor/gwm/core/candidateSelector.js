...
const CORE_ALPHA_PRIORITY_GAINS = Object.freeze([
  0.6, 1, 1.1, 1.15, 1.3, 0.45, 0.7, 0.85, 0.55,
]);
const CURRENT_LARGE_MARGIN_ULTRA_WEAK_ALPHA_GAINS = Object.freeze([
  0.25, 0.3, 0.35, 0.4,
]);
const STANDARD_ANCHOR_WEAK_ALPHA_RESCUE_GAINS = Object.freeze([
  0.55, 0.7, 0.85,
]);
...
function inferAlphaPriorityIndex(alphaGain) {
  const index = CORE_ALPHA_PRIORITY_GAINS.findIndex(
    (candidateGain) => Math.abs(candidateGain - alphaGain) < 0.0001,
  );
      ...
      for (const marginRightDelta of FIXED_CORE_LOCAL_MARGIN_DELTAS) {
        ...
        for (const marginBottomDelta of FIXED_CORE_LOCAL_MARGIN_DELTAS) {
          ...
          for (const alphaGain of alphaGainCandidates) {
            ...
            bestCandidate = pickBetterCandidate(
              bestCandidate,
              candidate,
              0.002,
            );
...
function promoteBaseCandidate(
  ...
  baseDecisionTier,
  candidate,
  { reliableMatch = false, minCostDelta = 0.002 } = {},
) {
