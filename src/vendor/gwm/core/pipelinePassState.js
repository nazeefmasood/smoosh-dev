...
export function applyPipelinePassOutcome({ current, outcome } = {}) {
  const passIncrement = Math.max(
    0,
    finiteOrFallback(outcome?.passIncrement, 0),
  );
  return {
    passCount: finiteOrFallback(current?.passCount, 0) + passIncrement,
    attemptedPassCount:
      finiteOrFallback(current?.attemptedPassCount, 0) + passIncrement,
