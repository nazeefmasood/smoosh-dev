...
async function createAllenkFdncnnOnnxRuntime({
  ...
  if (executionProvider === 'webgpu' && !ort) {
    throw new Error(
      'allenk FDnCNN WebGPU runtime requires an injected onnxruntime-web/webgpu module',
    );
  }
  const resolvedOrt = ort || wasmOrt;
  const resolvedInputShape = normalizeShape(inputShape, [1, 4, 72, 72]);
  const resolvedOutputShape = normalizeShape(outputShape, [
    1,
    3,
    resolvedInputShape[2],
    resolvedInputShape[3],
  ]);
  if (resolvedOrt.env?.wasm) {
    ...
    const resolvedNumThreads =
      numThreads === 'auto'
        ? canUseThreads
          ? Math.max(1, Math.min(4, Math.ceil(hardwareThreads / 2)))
          : 1
        : Math.max(1, Math.round(Number(numThreads) || 1));
    resolvedOrt.env.wasm.numThreads =
      executionProvider === 'wasm' ? resolvedNumThreads : 1;
  ...
  const resolvedSession =
    session ||
    (await resolvedOrt.InferenceSession.create(modelBytes, {
  ...
  return {
    ...
    async execute(input) {
      const tensor = new resolvedOrt.Tensor(
        'float32',
        input,
        resolvedInputShape,
      );
      ...
      if (!outputTensor?.data) {
        throw new Error(
          `allenk FDnCNN ONNX runtime did not return ${outputName}`,
        );
      }
      return {
...
export { createAllenkFdncnnOnnxRuntime, validateImageShape };
