...
export function normalizeAlphaAdjustmentStageForTrace(stagePayload = {}) {
  ...
  if (
    ...
    return null;
  if (!allowSameAlphaGain && Math.abs(fromAlphaGain - toAlphaGain) < 0.0001)
    return null;
...
export function createAlphaTraceContractSummary({
  ...
  return {
    alphaAdjustmentStageCount: Array.isArray(alphaAdjustmentStages)
      ? alphaAdjustmentStages.length
      : 0,
    alphaTrialEventCount: Array.isArray(alphaTrialEvents)
      ? alphaTrialEvents.length
      : 0,
    hasAlphaAdjustments:
      Array.isArray(alphaAdjustmentStages) && alphaAdjustmentStages.length > 0,
    hasAlphaTrialEvents:
      Array.isArray(alphaTrialEvents) && alphaTrialEvents.length > 0,
  };
}
